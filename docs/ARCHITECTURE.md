...
| `display.{h,cpp}` | **Portrait engine**: logical→panel transform, primitives, custom rotated/scaled text + icon blitter |
| `icons.h` | Material Design Icons subset (~56 glyphs: weather + ~45 selectable tile icons) as a GFXfont, generated by `fontconvert.py` |
...
| `homeassistant.{h,cpp}` | Home mode: configurable metric tiles (type catalog: icon+unit+decimals) pulled from HA REST, drawn as a 2×2 grid |
...
- **Icons** — Material Design Icons are subset into a GFXfont (`icons.h`, via the
  same `fontconvert.py` that made FiraSans) and drawn through the same blitter via
  `Display::icon(codepoint, cx, cy, scale)` — so weather/room icons are crisp,
  anti-aliased and scalable just like text. To add/change icons: edit the
  codepoint list, re-run `fontconvert.py` on `materialdesignicons-webfont.ttf`.
